  ...
  return (
    <Card className="stack-sm">
      <div className="row-between">
        <div className="card-title" style={{ margin: 0 }}>{t("导入对账单")}</div>
        <button className="btn-icon" onClick={onClose}><X size={15} /></button>
      </div>
      <div className="tiny faint">
        {t("把银行的 Statement of Account 里的表格整段复制粘贴进来（日期 / 代码 / 金额三列）。 认识 BAL-BF、RES、IP-2.60、AXS-L、INT-R、CPF、CPF-A、CPF-L、AXS-I、BAL-CF 这些代码。 导入前会用对账单自己打印的期末余额校验一遍，对不上会告诉你差多少。")}
      </div>
      <textarea
      ...
      <div className="row">
        ...
        {parsed && <button className="btn btn-sm" onClick={() => { setParsed(null); setText(""); }}>{t("重新粘贴")}</button>}
      </div>

      {done && (
        <Note>{t("导入完成：新增 {added} 条记录，跳过 {skipped} 条重复。", { added: done.added, skipped: done.skipped })}</Note>
      )}

      {parsed && (
        <>
          <div className="tiny">
            {t("解析出")} <b>{parsed.rows.length}</b> {t("条记录")}
            {parsed.rate ? ` · ${t("利率 {rate}", { rate: pct(parsed.rate) })}` : ""}
            {parsed.opening ? ` · ${t("期初 {amount}（{date}）", { amount: formatMoney(parsed.opening.amount, acc.currency), date: parsed.opening.date })}` : ""}
            {parsed.closing ? ` · ${t("期末 {amount}（{date}）", { amount: formatMoney(parsed.closing.amount, acc.currency), date: parsed.closing.date })}` : ""}
          </div>
          {recon && <Note tone={recon.ok ? "info" : "error"}>{t(recon.message)}</Note>}
          {parsed.interest.checked > 0 && (
            <div className="tiny faint">
              {t("利息复算：{matched}/{checked} 条与「余额 × 利率 ÷ 12」一致", { matched: parsed.interest.matched, checked: parsed.interest.checked })}
              {parsed.interest.mismatches.length > 0 && (
                <> —— {t("对不上的：{list}", {
                  list: parsed.interest.mismatches.slice(0, 3)
                    .map((m) => t("{date} 打印 {stated} / 算出 {expected}", { date: m.date, stated: m.stated.toFixed(2), expected: m.expected.toFixed(2) }))
                    .join("；"),
                })}</>
              )}
            </div>
          ...
          {parsed.unknown.length > 0 && (
            <Note tone="warn">
              {t("有 {n} 行没认出来，不会被导入：", { n: parsed.unknown.length })}
              <ul style={{ margin: "4px 0 0", paddingLeft: 18 }}>
                {parsed.unknown.slice(0, 5).map((u, i) => (
                  <li key={i} className="tiny">
                    {t("第 {line} 行 · {reason} · ", { line: u.line, reason: u.reason })}
                    <span className="mono-inline">{u.text.slice(0, 60)}</span>
                  </li>
                ))}
              </ul>
            </Note>
          ...
          {parsed.signConflicts.length > 0 && (
            <Note tone="warn">
              {t("有 {n} 条记录的正负号和它的代码含义不一致（例如本该是还款却是正数），导入后余额可能对不上。", { n: parsed.signConflicts.length })}
            </Note>
          )}
          {Object.keys(suggestedLoanSettings(parsed)).length > 0 && (
            <label className="check tiny">
              ...
              {t("同时把账户设置改成对账单里的值（{list}）", {
                list: Object.entries(suggestedLoanSettings(parsed)).map(([k, v]) => `${k}=${v}`).join("，"),
              })}
            </label>
          )}
          {blocked && (
          ...
          <div className="row">
            <button className="btn btn-primary" onClick={doImport} disabled={blocked || !parsed.rows.length}>
              <Check size={13} />{t("确认导入 {n} 条", { n: parsed.rows.length })}
            </button>
          </div>
        </>
      )}
...
function EntryForm({ acc, editing, timeline, onSubmit, onCancel }) {
  ...
  function submit() {
    ...
    onSubmit({
      ...
      rebate: !editing && isPrepay && f.withRebate && Number(rebateValue) > 0
        ? { id: undefined, type: "rebate", date: f.date, amount: round2(Number(rebateValue)), rate: 0, note: t("提前还款利息回扣") }
        : null,
    });
  ...
  return (
    <div className="stack-sm">
      <div className="card-title" style={{ margin: 0 }}>{editing ? t("修改记录") : t("记一笔")}</div>
      <div className="grid-form">
        <Field label={t("类型")}>
          <select className="select" value={f.type} onChange={(e) => setF({ ...f, type: e.target.value })}>
            {MANUAL_ENTRY_TYPES.map((k) => (
              <option key={k} value={k}>{t(LOAN_ENTRY_TYPES[k].label)}（{LOAN_ENTRY_TYPES[k].code}）</option>
            ))}
            {editing && editing.type === "interest" && <option value="interest">{t("月度利息（IP）")}</option>}
          </select>
        </Field>
        <Field label={t("日期")}>
        ...
        {isRate ? (
          ...
          <Field label={f.type === "set_balance" ? t("校准后的余额") : t("金额")}>
            <input className="input num" type="number" step="0.01" value={f.amount || ""}
              onChange={(e) => setF({ ...f, amount: e.target.value })} />
          </Field>
        )}
        <Field label={t("备注")}>
          <input className="input" value={f.note || ""} placeholder={t("可留空")}
          ...
          {f.withRebate && (
            <>
              ...
              <span className="tiny faint">
                {t("当月已按月初余额收过整月利息，中途还款会退回剩余天数那部分：还款额 × {rate} ÷ 12 × 当月剩余天数 ÷ 当月天数", { rate: pct(rateNow) })}
              </span>
            </>
          )}
        </div>
      ...
      <div className="row">
        <button className="btn btn-primary btn-sm" onClick={submit}>
          {editing ? <><Check size={13} />{t("保存修改")}</> : <><Plus size={13} />{t("记入")}</>}
        </button>
        {editing && <button className="btn btn-sm" onClick={onCancel}>{t("取消")}</button>}
        {!editing && (
          <span className="tiny faint">
            {t("每月 {day} 号的利息不用记 —— 会自动按余额 × 利率 ÷ 12 算出来。", { day: acc.accrualDay || 1 })}
          </span>
        )}
      </div>
    </div>
...
export default function LoanLedger({ account: acc, patch, asOf = todayISO() }) {
  ...
  const tl = useMemo(() => loanTimeline(acc, asOf), [acc, asOf]);
  const years = useMemo(
    () => Array.from(new Set(tl.rows.map((r) => r.date.slice(0, 4)))).sort().reverse(),
    [tl.rows]
  );
  const yearRows = useMemo(() => {
    if (year === "全部") return tl.rows;
    if (year === "最近") return tl.rows.slice(-15);
    return tl.rows.filter((r) => r.date.slice(0, 4) === year);
  }, [tl.rows, year]);
  const summaries = useMemo(() => loanYearSummary(tl), [tl]);
  const thisYear = summaries.find((s) => s.year === year);
  const payoff = loanPayoff(tl.balance, tl.rate, acc.monthlyPayment);
  ...
  return (
    <div className="stack-sm">
      <div className="grid-4">
        <Metric label={t("当前欠款")} value={formatMoney(tl.balance, currency)} tone="neg"
          sub={t("利率 {rate} · 已计息 {months} 个月", { rate: pct(tl.rate), months: tl.monthsAccrued })} />
        <Metric label={t("累计利息")} value={formatMoney(tl.totalInterest, currency)} tone="neg"
          sub={tl.totals.rebate ? t("已退回回扣 {amount}", { amount: formatMoney(tl.totals.rebate, currency) }) : ""} />
        <Metric label={t("累计还款")} value={formatMoney(tl.totalPaid, currency)}
          sub={t("月供 {instalment} · 提前还 {prepayment}", { instalment: formatMoney(tl.totals.instalment, currency), prepayment: formatMoney(tl.totals.prepayment, currency) })} />
        <Metric label={t("下次计息（{date}）", { date: tl.nextAccrualDate || "—" })}
          value={formatMoney(tl.nextAccrualInterest, currency)} tone="neg"
          sub={t("每月 {day} 号 · 余额 × {rate} ÷ 12", { day: acc.accrualDay || 1, rate: pct(tl.rate) })} />
      </div>

      {tl.balance > 0 && (Number(acc.monthlyPayment) || 0) > 0 && (
        payoff.enough ? (
          <div className="tiny faint">
            {t("只按月供 {payment} 还，还需 {months} 个月 （约 {years} 年，到 {date}），期间利息 {interest}。你平时的提前还款会把这个数字大幅拉低。", {
              payment: formatMoney(acc.monthlyPayment, currency),
              months: payoff.months,
              years: (payoff.months / 12).toFixed(1),
              date: addMonthsISO(asOf, payoff.months),
              interest: formatMoney(payoff.totalInterest, currency),
            })}
          </div>
        ) : (
          <Note tone="warn">{t("月供还不够覆盖每月利息，这样下去余额只会越滚越大。")}</Note>
        )
      )}
      ...
      {importing
        ...
        : (
          <div className="row">
            <button className="btn btn-sm" onClick={() => setImporting(true)}><FileUp size={13} />{t("导入对账单")}</button>
            <span className="tiny faint">{t("有银行的 Statement of Account 就直接粘贴，不用一条条敲。")}</span>
          </div>
        )}

      <div style={{ borderTop: "1px solid var(--border-soft)", paddingTop: 10 }}>
        <EntryForm acc={acc} editing={editing} timeline={tl}
      ...
      {tl.rows.length > 0 && (
        <>
          <div className="row-between" style={{ marginTop: 6 }}>
            <div className="card-title" style={{ margin: 0 }}>{t("完整记录")}</div>
            <div className="row">
              {["最近", "全部", ...years].map((y) => (
                <button key={y} className={`btn btn-sm ${year === y ? "btn-primary" : ""}`} onClick={() => setYear(y)}>
                  {y === "最近" ? t("最近 15 条") : y === "全部" ? t("全部 {n} 条", { n: tl.rows.length }) : t("{y} 年", { y })}
                </button>
              ))}
            </div>
          </div>
          ...
          {thisYear && (
            <div className="row tiny faint">
              <span>{t("{year} 年：", { year: thisYear.year })}</span>
              <span>{t("利息")} <span className="num neg">{formatMoney(thisYear.interest, currency)}</span></span>
              <span>{t("月供")} <span className="num">{formatMoney(thisYear.instalment, currency)}</span></span>
              <span>{t("提前还款")} <span className="num">{formatMoney(thisYear.prepayment, currency)}</span></span>
              <span>{t("回扣")} <span className="num pos">{formatMoney(thisYear.rebate, currency)}</span></span>
              <span>{t("年末余额")} <span className="num">{formatMoney(thisYear.closing, currency)}</span></span>
            </div>
          )}
          ...
          <div className="table-wrap" style={{ maxHeight: 460, overflowY: "auto" }}>
            <table className="tbl">
              <thead>
                <tr>
                  <th>{t("日期")}</th><th>{t("代码")}</th><th>{t("项目")}</th>
                  <th className="ta-r">{t("金额")}</th><th className="ta-r">{t("余额")}</th><th></th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={r.id || `${r.type}-${r.date}-${i}`}>
                      ...
                      <td className="nowrap">
                        ...
                        {r.generated ? <span className="tiny faint"> · {t("自动")}</span> : null}
                        {r.note ? <span className="faint small"> · {r.note}</span> : null}
                        {r.short ? <span className="warn small"> · {t("超出余额部分未计入")}</span> : null}
                      </td>
                      ...
                      <td className="nowrap">
                        <button className="btn-icon" title={r.generated ? t("改成手动填写的金额") : t("编辑")}
                          style={{ color: "var(--ink-faint)" }} onClick={() => startEdit(r)}>
                          <Pencil size={12} />
                        </button>
                        {!r.generated && (
                          <button className="btn-icon" title={t("删除")} onClick={() => setConfirmDelete(r.id)}>
                            <Trash2 size={12} />
                          </button>
                        )}
                      </td>
          ...
          {confirmDelete && (
            <div className="note note-warn" style={{ justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <div>{t("删除这条记录？之后的余额会重新算过。")}</div>
              </div>
              <div className="row" style={{ flexWrap: "nowrap" }}>
                <button className="btn btn-sm" onClick={() => setConfirmDelete(null)}>{t("取消")}</button>
                <button className="btn btn-sm btn-danger" onClick={() => removeEntry(confirmDelete)}>{t("删除")}</button>
              </div>
            </div>
          )}
        </>
