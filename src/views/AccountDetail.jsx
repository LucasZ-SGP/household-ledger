...
function CashByCurrency({ acc, patch, currencies }) {
  ...
  return (
    <div className="stack-sm">
      <div className="field-label">闲散资金（各币种分开记，不换算）</div>
      {shown.length === 0 && <div className="tiny faint">还没记任何现金。</div>}
      {shown.map((ccy) => (
        <div className="row" key={ccy} style={{ flexWrap: "nowrap", maxWidth: 320 }}>
          ...
          <button className="btn-icon" title={`删除 ${ccy}`} onClick={() => remove(ccy)}><Trash2 size={13} /></button>
        </div>
      ))}
      {addable.length > 0 && (
        <div className="row">
        ...
        <table className="tbl">
          <thead>
            <tr>
              ...
              <th className="ta-r">现价</th><th className="ta-r">市值</th><th></th>
            </tr>
          </thead>
          <tbody>
            {(acc.holdings || []).map((h) => {
            ...
            {!(acc.holdings || []).length && (
              <tr><td colSpan={6} className="faint small" style={{ textAlign: "center", padding: 18 }}>还没有持仓</td></tr>
            )}
          </tbody>
        </table>
      </div>
    ...
    <div className="stack-sm">
      <Field label="说明">
        <textarea className="input" rows={3}
          placeholder={acc.kind === "vehicle" ? "例如：本田 CR-V，COE 2032 年到期" : "例如：Punggol 四房，2024 年入伙"}
          value={acc.description || ""} onChange={(e) => patch({ description: e.target.value })} />
      </Field>
      <div className="tiny faint">
        这里只记名字和说明，不记金额 —— 房子和车的「现值」本来就只是个估计，
        一旦写进净资产合计，过几个月就会变成一个没人记得更新、却看起来很确定的数字。
        真要估值的时候，用「其他资产」记一笔就好。
      </div>
    </div>
  );
}
